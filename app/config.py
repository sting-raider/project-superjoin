...
class Settings:
    ...
    extraction_max_output_tokens: int = _int_env("EXTRACTION_MAX_OUTPUT_TOKENS", 2400)
    ...
    extraction_batch_pages: int = _int_env("EXTRACTION_BATCH_PAGES", 3)
    ...
    extraction_extra_body_json: str = _env("EXTRACTION_EXTRA_BODY_JSON")
    reasoning_extra_body_json: str = _env("REASONING_EXTRA_BODY_JSON")
    vision_extra_body_json: str = _env("VISION_EXTRA_BODY_JSON")
    embedding_extra_body_json: str = _env("EMBEDDING_EXTRA_BODY_JSON")
    provider_retry_attempts: int = _int_env("AI_RETRY_ATTEMPTS", 3)
    provider_retry_backoff_seconds: float = _float_env("AI_RETRY_BACKOFF_SECONDS", 0.5)
