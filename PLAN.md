...
- [ ] The project is named **Project SuperJoin** throughout the UI, README, documentation, and demo.
- [ ] Clean clone starts with `docker compose up --build` and no `.env` through the documented lawful source-provisioning path.
- [ ] No-key demo works without runtime network access after provisioning.
- [ ] All six starter documents have page-level processing coverage.
- [ ] Starter PDFs and derivative artifacts were audited before public commits/releases.
- [ ] No unapproved third-party material appears in Git history, release assets, or published images.
- [ ] Demo data was generated by the pipeline; replay is explicitly labeled.
- [ ] A never-seen PDF processes through a configured compatible endpoint.
- [ ] Extraction, reasoning, vision, and embeddings can be configured independently without code changes.
...
- [ ] Every canonical supporting claim has valid, inspectable evidence.
- [ ] All four assignment cases are accessible in one click.
- [ ] Numeric and semantic temporal cases both work.
- [ ] Missing context, incompatible embeddings, and unsupported capabilities fail visibly.
- [ ] Conflicts and inadequate evidence cannot silently pass the strict Trust Gate.
- [ ] Native and visual document prompt-injection fixtures cannot override instructions, verification, or strict fact resolution.
- [ ] Human decisions are immutable/auditable, revocable, and invalidated appropriately by new evidence.
- [ ] Duplicate ingestion makes no paid calls; incremental ingestion preserves previous work.
...
- [ ] Any unrun 1,000-page/50,000-claim benchmarks are labeled accurately and do not block the finished core submission.
- [ ] Cumulative paid API usage remains within US$20.
- [ ] Relevant tests, type checks, accessibility checks, and clean-build checks pass.
- [ ] Repository contains no credentials, unnecessary renders, temporary files, or fabricated outputs.
- [ ] Small, coherent, tested changes were committed and pushed frequently throughout implementation.
- [ ] No giant final commit substitutes for incremental development history.
- [ ] All completed implementation work is committed and pushed to `main`.
