...
def test_profile_live_run_aggregates_network_calls_and_elapsed_time() -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE model_calls(run_id TEXT,role TEXT,status TEXT,request_chars INTEGER,input_tokens INTEGER,
        ...
        CREATE TABLE claims(document_id TEXT,extraction_status TEXT);
        ...
        INSERT INTO claims VALUES('d','accepted');
        INSERT INTO model_calls VALUES('r','extraction','complete',500,100,20,500,1,0,0.1);
        INSERT INTO model_calls VALUES('r','extraction','cache_hit',0,0,0,0,1,1,0);
    ...
    complete = next(row for row in report["model_calls"] if row["status"] == "complete")
    assert complete["request_chars"] == 500
