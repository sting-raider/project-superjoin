...
The full starter-corpus model-quality comparison and 1,000-page/50,000-claim synthetic performance run remain bounded follow-up work requiring additional provider coverage or source-verified inputs. The parser, extraction compatibility/selection, and embedding endpoint/retrieval evidence above are reproducible harnesses rather than hidden assumptions. The README explains how to supply the archive locally without redistributing PDFs whose rights have not been audited.
