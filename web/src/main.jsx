...
function Settings() {
  ...
  return <div className="content"><div className="section-heading"><div><span className="eyebrow">RUNTIME CONFIGURATION</span><h2>Settings</h2><p>Each role accepts any configured OpenAI-compatible endpoint and model.</p></div></div><div className="settings-grid">{roleCards.map(([key, label]) => { const role = settings?.roles?.[key]; return <div className="setting-card" key={key}><div className="setting-head"><span className="eyebrow">{label}</span><Badge tone={role?.configured ? 'good' : 'warn'}>{role?.configured ? 'configured' : 'offline'}</Badge></div><strong>{role?.model || 'model not configured'}</strong><small>{role?.base_url || 'Endpoint not configured'}</small><small>{role?.key_configured ? 'API key configured' : 'No API key (local endpoints may omit one)'}</small><small>{key === 'embeddings' ? `${role?.dimensions || '—'} dimensions · ${role?.task_type || 'task unspecified'} · ${role?.timeout_seconds || '—'}s` : `${role?.max_output_tokens || '—'} output tokens · ${role?.timeout_seconds || '—'}s · ${role?.structured_output_mode || 'response'}`}</small></div> })}</div><div className="panel settings-note"><Badge tone={settings?.provider_configured ? 'good' : 'warn'}>{settings?.provider_configured ? 'Live provider configured' : 'Recorded demo mode'}</Badge><p>Secrets are never returned to this screen. Configure role-specific base URLs, keys, model names, paths, and limits in the environment; the bundled snapshot requires no key.</p><div className="budget-line"><span>Budget remaining</span><strong>${settings?.budget?.remaining_usd?.toFixed?.(4) || '20.0000'}</strong></div></div></div>
}
